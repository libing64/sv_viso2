...
void Matcher::pushBack(uint8_t* I1, uint8_t* I2, int32_t* dims,
                       const bool replace) {
  // image dimensions
  ...
  if (dims_c[2] == bpl) {
    ...
    if (I2 != 0) memcpy(I2c, I2, dims_c[2] * dims_c[1] * sizeof(uint8_t));
  } else {
  ...
  computeFeatures(I1c, dims_c, m1c1, n1c1, m1c2, n1c2, I1c_du, I1c_dv,
                  I1c_du_full, I1c_dv_full);
  if (I2 != 0)
    computeFeatures(I2c, dims_c, m2c1, n2c1, m2c2, n2c2, I2c_du, I2c_dv,
                    I2c_du_full, I2c_dv_full);
}
...
void Matcher::matchFeatures(int32_t method, Matrix* Tr_delta) {
  ...
  // flow
  if (method == 0) {
    if (m1p2 == 0 || n1p2 == 0 || m1c2 == 0 || n1c2 == 0) return;
    if (param.multi_stage)
      if (m1p1 == 0 || n1p1 == 0 || m1c1 == 0 || n1c1 == 0) return;

    // stereo
  } else if (method == 1) {
    if (m1c2 == 0 || n1c2 == 0 || m2c2 == 0 || n2c2 == 0) return;
    if (param.multi_stage)
      if (m1c1 == 0 || n1c1 == 0 || m2c1 == 0 || n2c1 == 0) return;

    // quad matching
  } else {
    if (m1p2 == 0 || n1p2 == 0 || m2p2 == 0 || n2p2 == 0 || m1c2 == 0 ||
        n1c2 == 0 || m2c2 == 0 || n2c2 == 0)
      return;
    if (param.multi_stage)
      if (m1p1 == 0 || n1p1 == 0 || m2p1 == 0 || n2p1 == 0 || m1c1 == 0 ||
          n1c1 == 0 || m2c1 == 0 || n2c1 == 0)
        return;
  }
  ...
  if (param.multi_stage) {
    ...
    matching(m1p1, m2p1, m1c1, m2c1, n1p1, n2p1, n1c1, n2c1, p_matched_1,
             method, false, Tr_delta);
    ...
    matching(m1p2, m2p2, m1c2, m2c2, n1p2, n2p2, n1c2, n2c2, p_matched_2,
             method, true, Tr_delta);
    if (param.refinement > 0) refinement(p_matched_2, method);
...
void Matcher::bucketFeatures(int32_t max_features, float bucket_width,
                             float bucket_height) {
  // find max values
  float u_max = 0;
  float v_max = 0;
  for (vector<p_match>::iterator it = p_matched_2.begin();
  ...
  for (int32_t i = 0; i < bucket_cols * bucket_rows; i++) {
    ...
    int32_t k = 0;
    for (vector<p_match>::iterator it = buckets[i].begin();
         it != buckets[i].end(); it++) {
      p_matched_2.push_back(*it);
      k++;
      if (k >= max_features) break;
    }
  }
...
float Matcher::getGain(vector<int32_t> inliers) {
  ...
  for (vector<int32_t>::iterator it = inliers.begin(); it != inliers.end();
       it++) {
    if (*it < (int32_t)p_matched_2.size()) {
      // mean in previous image
      u_min =
          min(max((int32_t)p_matched_2[*it].u1p - window_size, 0), dims_p[0]);
      u_max =
          min(max((int32_t)p_matched_2[*it].u1p + window_size, 0), dims_p[0]);
      v_min =
          min(max((int32_t)p_matched_2[*it].v1p - window_size, 0), dims_p[1]);
      v_max =
          min(max((int32_t)p_matched_2[*it].v1p + window_size, 0), dims_p[1]);
      ...
      u_min =
          min(max((int32_t)p_matched_2[*it].u1c - window_size, 0), dims_p[0]);
      u_max =
          min(max((int32_t)p_matched_2[*it].u1c + window_size, 0), dims_p[0]);
      v_min =
          min(max((int32_t)p_matched_2[*it].v1c - window_size, 0), dims_p[1]);
      v_max =
          min(max((int32_t)p_matched_2[*it].v1c + window_size, 0), dims_p[1]);
...
void Matcher::nonMaximumSuppression(int16_t* I_f1, int16_t* I_f2,
  ...
  for (int32_t i = n + margin; i < width - n - margin; i += n + 1) {
    for (int32_t j = n + margin; j < height - n - margin; j += n + 1) {
      ...
      // f1 minimum
      for (int32_t i2 = f1mini - n; i2 <= min(f1mini + n, width - 1 - margin);
           i2++) {
        for (int32_t j2 = f1minj - n;
             j2 <= min(f1minj + n, height - 1 - margin); j2++) {
    ...
    failed_f1min:
      ...
      // f1 maximum
      for (int32_t i2 = f1maxi - n; i2 <= min(f1maxi + n, width - 1 - margin);
           i2++) {
        for (int32_t j2 = f1maxj - n;
             j2 <= min(f1maxj + n, height - 1 - margin); j2++) {
          currval = *(I_f1 + getAddressOffsetImage(i2, j2, bpl));
          if (currval > f1maxval &&
              (i2 < i || i2 > i + n || j2 < j || j2 > j + n))
            goto failed_f1max;
    ...
    failed_f1max:
      ...
      // f2 minimum
      for (int32_t i2 = f2mini - n; i2 <= min(f2mini + n, width - 1 - margin);
           i2++) {
        for (int32_t j2 = f2minj - n;
             j2 <= min(f2minj + n, height - 1 - margin); j2++) {
    ...
    failed_f2min:
      ...
      // f2 maximum
      for (int32_t i2 = f2maxi - n; i2 <= min(f2maxi + n, width - 1 - margin);
           i2++) {
        for (int32_t j2 = f2maxj - n;
             j2 <= min(f2maxj + n, height - 1 - margin); j2++) {
          currval = *(I_f2 + getAddressOffsetImage(i2, j2, bpl));
          if (currval > f2maxval &&
              (i2 < i || i2 > i + n || j2 < j || j2 > j + n))
            goto failed_f2max;
...
inline void Matcher::computeSmallDescriptor(const uint8_t* I_du,
                                            const uint8_t* I_dv,
                                            const int32_t& bpl,
...
void Matcher::computeDescriptors(uint8_t* I_du, uint8_t* I_dv,
                                 const int32_t bpl,
  ...
  // for all maxima do
  for (vector<Matcher::maximum>::iterator it = maxima.begin();
       it != maxima.end(); it++) {
    u = (*it).u;
    v = (*it).v;
...
void Matcher::filterImageAll(uint8_t* I, uint8_t* I_du, uint8_t* I_dv,
  ...
  for (; p44 != end_input; p00++, p01++, p02++, p03++, p04++, p10++, p11++,
                           p12++, p13++, p14++, p20++, p21++, p22++, p23++,
                           p24++, p30++, p31++, p32++, p33++, p34++, p40++,
                           p41++, p42++, p43++, p44++, result_du++, result_dv++,
                           result_f1++, result_f2++) {
    ...
    *result_f1 = -*p00 - *p01 - *p02 - *p03 - *p04 - *p10 + *p11 + *p12 + *p13 -
                 *p14 - *p20 + *p21 + 8 * *p22 + *p23 - *p24 - *p30 + *p31 +
                 *p32 + *p33 - *p34 - *p40 - *p41 - *p42 - *p43 - *p44;
    *result_f2 = -*p00 - *p01 + *p03 + *p04 - *p10 - *p11 + *p13 + *p14 + *p30 +
                 *p31 - *p33 - *p34 + *p40 + *p41 - *p43 - *p44;
  }
}

void Matcher::filterImageSobel(uint8_t* I, uint8_t* I_du, uint8_t* I_dv,
                               const int* dims) {
  ...
  // compute filter responses (border pixels are invalid)
  for (; p22 != end_input; p00++, p01++, p02++, p10++, p11++, p12++, p20++,
                           p21++, p22++, result_du++, result_dv++) {
    int16_t temp_du = -*p00 - 2 * *p10 - *p20 + *p02 + 2 * *p12 + *p22;
    int16_t temp_dv = -*p00 - 2 * *p01 - *p02 + *p20 + 2 * *p21 + *p22;
    *result_du = saturate(temp_du / 4 + 128);
    *result_dv = saturate(temp_dv / 4 + 128);
...
uint8_t* Matcher::createHalfResolutionImage(uint8_t* I, const int32_t* dims) {
  ...
  uint8_t* I_half =
      (uint8_t*)_mm_malloc(dims_half[2] * dims_half[1] * sizeof(uint8_t), 16);
  for (int32_t v = 0; v < dims_half[1]; v++)
    for (int32_t u = 0; u < dims_half[0]; u++)
      I_half[v * dims_half[2] + u] =
          (uint8_t)(((int32_t)I[(v * 2 + 0) * dims[2] + u * 2 + 0] +
...
void Matcher::computeFeatures(uint8_t* I, const int32_t* dims, int32_t*& max1,
  ...
  if (!param.half_resolution) {
    ...
    I_du = (uint8_t*)_mm_malloc(
        dims_matching[2] * dims_matching[1] * sizeof(uint8_t*), 16);
    I_dv = (uint8_t*)_mm_malloc(
        dims_matching[2] * dims_matching[1] * sizeof(uint8_t*), 16);
    I_f1 = (int16_t*)_mm_malloc(
        dims_matching[2] * dims_matching[1] * sizeof(int16_t), 16);
    I_f2 = (int16_t*)_mm_malloc(
        dims_matching[2] * dims_matching[1] * sizeof(int16_t), 16);
    I_du_full = (uint8_t*)_mm_malloc(dims[2] * dims[1] * sizeof(uint8_t*), 16);
    I_dv_full = (uint8_t*)_mm_malloc(dims[2] * dims[1] * sizeof(uint8_t*), 16);
    filter::sobel5x5(I_matching, I_du, I_dv, dims_matching[2],
                     dims_matching[1]);
    filter::sobel5x5(I, I_du_full, I_dv_full, dims[2], dims[1]);
    filter::blob5x5(I_matching, I_f1, dims_matching[2], dims_matching[1]);
    filter::checkerboard5x5(I_matching, I_f2, dims_matching[2],
                            dims_matching[1]);
    _mm_free(I_matching);
  }
  ...
  if (num1 != 0) {
    ...
    int32_t k = 0;
    for (vector<Matcher::maximum>::iterator it = maxima1.begin();
         it != maxima1.end(); it++) {
      *(max1 + k++) = it->u * s;
      *(max1 + k++) = it->v * s;
      *(max1 + k++) = 0;
  ...
  if (num2 != 0) {
    ...
    int32_t k = 0;
    for (vector<Matcher::maximum>::iterator it = maxima2.begin();
         it != maxima2.end(); it++) {
      *(max2 + k++) = it->u * s;
      *(max2 + k++) = it->v * s;
      *(max2 + k++) = 0;
...
void Matcher::computePriorStatistics(vector<Matcher::p_match>& p_matched,
  ...
  int32_t u_bin_num =
      (int32_t)ceil((float)dims_c[0] / (float)param.match_binsize);
  int32_t v_bin_num =
      (int32_t)ceil((float)dims_c[1] / (float)param.match_binsize);
  ...
  for (vector<Matcher::p_match>::iterator it = p_matched.begin();
    ...
    if (method < 2) {
      u_bin_min =
          min(max((int32_t)floor(it->u1c / (float)param.match_binsize) - 1, 0),
              u_bin_num - 1);
      u_bin_max =
          min(max((int32_t)floor(it->u1c / (float)param.match_binsize) + 1, 0),
              u_bin_num - 1);
      v_bin_min =
          min(max((int32_t)floor(it->v1c / (float)param.match_binsize) - 1, 0),
              v_bin_num - 1);
      v_bin_max =
          min(max((int32_t)floor(it->v1c / (float)param.match_binsize) + 1, 0),
              v_bin_num - 1);
      ...
      u_bin_min =
          min(max((int32_t)floor(it->u1p / (float)param.match_binsize) - 1, 0),
              u_bin_num - 1);
      u_bin_max =
          min(max((int32_t)floor(it->u1p / (float)param.match_binsize) + 1, 0),
              u_bin_num - 1);
      v_bin_min =
          min(max((int32_t)floor(it->v1p / (float)param.match_binsize) - 1, 0),
              v_bin_num - 1);
      v_bin_max =
          min(max((int32_t)floor(it->v1p / (float)param.match_binsize) + 1, 0),
              v_bin_num - 1);
  ...
  for (int32_t v_bin = 0; v_bin < v_bin_num; v_bin++) {
    for (int32_t u_bin = 0; u_bin < u_bin_num; u_bin++) {
      ...
      if (delta_accu[v_bin * u_bin_num + u_bin].size() > 0) {
        ...
        for (vector<Matcher::delta>::iterator it =
                 delta_accu[v_bin * u_bin_num + u_bin].begin();
...
void Matcher::createIndexVector(int32_t* m, int32_t n, vector<int32_t>* k,
  ...
  for (int32_t i = 0; i < n; i++) {
    ...
    // compute row and column of bin to which this observation belongs
    int32_t u_bin = min((int32_t)floor((float)u / (float)param.match_binsize),
                        u_bin_num - 1);
    int32_t v_bin = min((int32_t)floor((float)v / (float)param.match_binsize),
                        v_bin_num - 1);
...
inline void Matcher::findMatch(int32_t* m1, const int32_t& i1, int32_t* m2,
                               ...
                               int32_t stage, bool flow, bool use_prior,
                               double u_, double v_) {
  ...
  // bins of interest
  int32_t u_bin_min =
      min(max((int32_t)floor(u_min / (float)param.match_binsize), 0),
          u_bin_num - 1);
  int32_t u_bin_max =
      min(max((int32_t)floor(u_max / (float)param.match_binsize), 0),
          u_bin_num - 1);
  int32_t v_bin_min =
      min(max((int32_t)floor(v_min / (float)param.match_binsize), 0),
          v_bin_num - 1);
  int32_t v_bin_max =
      min(max((int32_t)floor(v_max / (float)param.match_binsize), 0),
          v_bin_num - 1);
  ...
  for (int32_t u_bin = u_bin_min; u_bin <= u_bin_max; u_bin++) {
    for (int32_t v_bin = v_bin_min; v_bin <= v_bin_max; v_bin++) {
      ...
      for (vector<int32_t>::const_iterator i2_it = k2[k2_ind].begin();
           i2_it != k2[k2_ind].end(); i2_it++) {
        int32_t u2 = *(m2 + step_size * (*i2_it) + 0);
        int32_t v2 = *(m2 + step_size * (*i2_it) + 1);
        if (u2 >= u_min && u2 <= u_max && v2 >= v_min && v2 <= v_max) {
          __m128i xmm3 =
              _mm_load_si128((__m128i*)(m2 + step_size * (*i2_it) + 4));
          __m128i xmm4 =
              _mm_load_si128((__m128i*)(m2 + step_size * (*i2_it) + 8));
          xmm3 = _mm_sad_epu8(xmm1, xmm3);
          xmm4 = _mm_sad_epu8(xmm2, xmm4);
          xmm4 = _mm_add_epi16(xmm3, xmm4);
          double cost =
              (double)(_mm_extract_epi16(xmm4, 0) + _mm_extract_epi16(xmm4, 4));
...
void Matcher::matching(int32_t* m1p, int32_t* m2p, int32_t* m1c, int32_t* m2c,
  ...
  if (method == 0) {
    ...
    for (i1c = 0; i1c < n1c; i1c++) {
      ...
      // compute row and column of statistics bin to which this observation
      // belongs
      int32_t u_bin =
          min((int32_t)floor((float)u1c / (float)param.match_binsize),
              u_bin_num - 1);
      int32_t v_bin =
          min((int32_t)floor((float)v1c / (float)param.match_binsize),
              v_bin_num - 1);
      int32_t stat_bin = v_bin * u_bin_num + u_bin;

      // match forward/backward
      findMatch(m1c, i1c, m1p, step_size, k1p, u_bin_num, v_bin_num, stat_bin,
                i1p, 0, true, use_prior);
      findMatch(m1p, i1p, m1c, step_size, k1c, u_bin_num, v_bin_num, stat_bin,
                i1c2, 1, true, use_prior);
      ...
      if (i1c2 == i1c) {
        ...
        if (*(M + getAddressOffsetImage(u1c, v1c, dims_c[0])) == 0) {
          p_matched.push_back(Matcher::p_match(u1p, v1p, i1p, -1, -1, -1, u1c,
                                               v1c, i1c, -1, -1, -1));
    ...
    for (i1c = 0; i1c < n1c; i1c++) {
      ...
      // match left/right
      findMatch(m1c, i1c, m2c, step_size, k2c, u_bin_num, v_bin_num, stat_bin,
                i2c, 0, false, use_prior);
      findMatch(m2c, i2c, m1c, step_size, k1c, u_bin_num, v_bin_num, stat_bin,
                i1c2, 1, false, use_prior);
      ...
      if (i1c2 == i1c) {
        ...
        if (u1c >= u2c) {
          // add match if this pixel isn't matched yet
          if (*(M + getAddressOffsetImage(u1c, v1c, dims_c[0])) == 0) {
            p_matched.push_back(Matcher::p_match(-1, -1, -1, -1, -1, -1, u1c,
                                                 v1c, i1c, u2c, v2c, i2c));
            *(M + getAddressOffsetImage(u1c, v1c, dims_c[0])) = 1;
          }
        }
    ...
    for (i1p = 0; i1p < n1p; i1p++) {
      ...
      // belongs
      int32_t u_bin =
          min((int32_t)floor((float)u1p / (float)param.match_binsize),
              u_bin_num - 1);
      int32_t v_bin =
          min((int32_t)floor((float)v1p / (float)param.match_binsize),
              v_bin_num - 1);
      int32_t stat_bin = v_bin * u_bin_num + u_bin;

      // match in circle
      findMatch(m1p, i1p, m2p, step_size, k2p, u_bin_num, v_bin_num, stat_bin,
                i2p, 0, false, use_prior);
      ...
      if (Tr_delta) {
        ...
        findMatch(m2p, i2p, m2c, step_size, k2c, u_bin_num, v_bin_num, stat_bin,
                  i2c, 1, true, use_prior, u2c_, v2c_);
      } else {
        findMatch(m2p, i2p, m2c, step_size, k2c, u_bin_num, v_bin_num, stat_bin,
                  i2c, 1, true, use_prior);
      }
      findMatch(m2c, i2c, m1c, step_size, k1c, u_bin_num, v_bin_num, stat_bin,
                i1c, 2, false, use_prior);
      if (Tr_delta)
        findMatch(m1c, i1c, m1p, step_size, k1p, u_bin_num, v_bin_num, stat_bin,
                  i1p2, 3, true, use_prior, u1p, v1p);
      else
        findMatch(m1c, i1c, m1p, step_size, k1p, u_bin_num, v_bin_num, stat_bin,
                  i1p2, 3, true, use_prior);
    ...
    belongs
      int32_t u_bin =
    min((int32_t)floor((float)u1c/(float)param.match_binsize),u_bin_num-1);
      int32_t v_bin =
    min((int32_t)floor((float)v1c/(float)param.match_binsize),v_bin_num-1);
      int32_t stat_bin = v_bin*u_bin_num+u_bin;

      // match in circle
      findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p,
    0,true ,use_prior,Tr_delta);
      findMatch(m1p,i1p,m2p,step_size,k2p,u_bin_num,v_bin_num,stat_bin,i2p,
    1,false,use_prior,Tr_delta);
      findMatch(m2p,i2p,m2c,step_size,k2c,u_bin_num,v_bin_num,stat_bin,i2c,
    2,true ,use_prior,Tr_delta);
...
void Matcher::removeOutliers(vector<Matcher::p_match>& p_matched,
  ...
  // markers)
  // attention: not using the B switch or using the n switch creates a memory
  // leak (=> use valgrind!)
  char parameters[] = "zQB";
  ...
  for (int i = 0; i < in.numberofpoints; i++)
    if (num_support[i] >= 4) p_matched.push_back(p_matched_copy[i]);
...
bool Matcher::parabolicFitting(const uint8_t* I1_du, const uint8_t* I1_dv,
                               const int32_t* dims1, const uint8_t* I2_du,
                               const uint8_t* I2_dv, const int32_t* dims2,
                               const float& u1, const float& v1, float& u2,
                               float& v2, Matrix At, Matrix AtA,
                               uint8_t* desc_buffer) {
  // check if parabolic fitting is feasible (descriptors are within margin)
  if (u2 - 3 < margin || u2 + 3 > dims2[0] - 1 - margin || v2 - 3 < margin ||
      v2 + 3 > dims2[1] - 1 - margin)
    return false;

  // compute reference descriptor
  __m128i xmm1, xmm2;
  computeSmallDescriptor(I1_du, I1_dv, dims1[2], (int32_t)u1, (int32_t)v1,
                         desc_buffer);
  ...
  for (int32_t dv = 0; dv < 7; dv++) {
    for (int32_t du = 0; du < 7; du++) {
      computeSmallDescriptor(I2_du, I2_dv, dims2[2], (int32_t)u2 + du - 3,
                             (int32_t)v2 + dv - 3, desc_buffer);
      xmm2 = _mm_load_si128((__m128i*)(desc_buffer));
      xmm2 = _mm_sad_epu8(xmm1, xmm2);
      cost[dv * 7 + du] =
          _mm_extract_epi16(xmm2, 0) + _mm_extract_epi16(xmm2, 4);
    }
  }
  ...
  if (fabs(divisor) < 1e-8 || fabs(b.val[2][0]) < 1e-8) return false;
  float ddv =
      (2.0 * b.val[0][0] * b.val[4][0] - b.val[2][0] * b.val[3][0]) / divisor;
...
void Matcher::relocateMinimum(const uint8_t* I1_du, const uint8_t* I1_dv,
                              const int32_t* dims1, const uint8_t* I2_du,
                              const uint8_t* I2_dv, const int32_t* dims2,
                              const float& u1, const float& v1, float& u2,
                              float& v2, uint8_t* desc_buffer) {
  // check if parabolic fitting is feasible (descriptors are within margin)
  if (u2 - 2 < margin || u2 + 2 > dims2[0] - 1 - margin || v2 - 2 < margin ||
      v2 + 2 > dims2[1] - 1 - margin)
    return;
  ...
  for (int32_t dv = 0; dv < 5; dv++) {
    for (int32_t du = 0; du < 5; du++) {
      computeSmallDescriptor(I2_du, I2_dv, dims2[2], (int32_t)u2 + du - 2,
                             (int32_t)v2 + dv - 2, desc_buffer);
      xmm2 = _mm_load_si128((__m128i*)(desc_buffer));
      xmm2 = _mm_sad_epu8(xmm1, xmm2);
      cost[dv * 5 + du] =
          _mm_extract_epi16(xmm2, 0) + _mm_extract_epi16(xmm2, 4);
...
void Matcher::refinement(vector<Matcher::p_match>& p_matched, int32_t method) {
  ...
  FLOAT A_data[9 * 6] = {1,  1, 1,  -1, -1, 1, 0, 1,  0, 0, -1, 1,  1, 1,
  ...
  // for all matches do
  for (vector<Matcher::p_match>::iterator it = p_matched_copy.begin();
       it != p_matched_copy.end(); it++) {
    // method: flow or quad matching
    if (method == 0 || method == 2) {
      if (param.refinement == 2) {
        if (!parabolicFitting(I1c_du_fit, I1c_dv_fit, dims_c, I1p_du_fit,
                              I1p_dv_fit, dims_p, it->u1c, it->v1c, it->u1p,
                              it->v1p, At, AtA, desc_buffer))
          continue;
      } else {
        relocateMinimum(I1c_du_fit, I1c_dv_fit, dims_c, I1p_du_fit, I1p_dv_fit,
                        dims_p, it->u1c, it->v1c, it->u1p, it->v1p,
                        desc_buffer);
      }
    }
    ...
    if (method == 1 || method == 2) {
      if (param.refinement == 2) {
        if (!parabolicFitting(I1c_du_fit, I1c_dv_fit, dims_c, I2c_du_fit,
                              I2c_dv_fit, dims_c, it->u1c, it->v1c, it->u2c,
                              it->v2c, At, AtA, desc_buffer))
          continue;
      } else {
        relocateMinimum(I1c_du_fit, I1c_dv_fit, dims_c, I2c_du_fit, I2c_dv_fit,
                        dims_c, it->u1c, it->v1c, it->u2c, it->v2c,
                        desc_buffer);
      }
    }
    ...
    if (method == 2) {
      if (param.refinement == 2) {
        if (!parabolicFitting(I1c_du_fit, I1c_dv_fit, dims_c, I2p_du_fit,
                              I2p_dv_fit, dims_p, it->u1c, it->v1c, it->u2p,
                              it->v2p, At, AtA, desc_buffer))
          continue;
      } else {
        relocateMinimum(I1c_du_fit, I1c_dv_fit, dims_c, I2p_du_fit, I2p_dv_fit,
                        dims_p, it->u1c, it->v1c, it->u2p, it->v2p,
                        desc_buffer);
      }
    }
