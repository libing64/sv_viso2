...
class OdometerBase {
 ...
 public:
  OdometerBase() {
    ...
    ROS_INFO_STREAM(
        ...
        << std::endl << "  odom_frame_id      = " << odom_frame_id_ << std::endl
    ...
    reset_service_ =
        local_nh.advertiseService("reset_pose", &OdometerBase::resetPose, this);
 ...
 protected:
  ...
  void integrateAndPublish(const tf::Transform& delta_transform,
    ...
    if (tf_listener_.canTransform(base_link_frame_id_, sensor_frame_id_,
                                  timestamp, &error_msg)) {
      tf_listener_.lookupTransform(base_link_frame_id_, sensor_frame_id_,
    ...
    tf::Transform base_transform =
        base_to_sensor * integrated_pose_ * base_to_sensor.inverse();
    ...
    // calculate twist (not possible for first run as no delta_t can be
    // computed)
    tf::Transform delta_base_transform =
        base_to_sensor * delta_transform * base_to_sensor.inverse();
    if (!last_update_time_.isZero()) {
      double delta_t = (timestamp - last_update_time_).toSec();
      if (delta_t) {
        odometry_msg.twist.twist.linear.x =
            delta_base_transform.getOrigin().getX() / delta_t;
        odometry_msg.twist.twist.linear.y =
            delta_base_transform.getOrigin().getY() / delta_t;
        odometry_msg.twist.twist.linear.z =
            delta_base_transform.getOrigin().getZ() / delta_t;
    ...
    if (publish_tf_) {
      tf_broadcaster_.sendTransform(tf::StampedTransform(
          base_transform, timestamp, odom_frame_id_, base_link_frame_id_));
    }
