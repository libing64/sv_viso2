...
class Reconstruction {
 public:
  ...
  // estimate between the 2 frames Tr, tries to associate the features with
  // previous
  // frames (tracking) and computes 3d points once tracks gets lost.
  // point types: 0 ..... everything
  //              1 ..... road and above
  //              2 ..... only above road
  // min_track_length ... min number of frames a point needs to be tracked for
  // reconstruction
  // max_dist ........... maximum point distance from camera
  // min_angle .......... avoid angles smaller than this for reconstruction (in
  // degrees)
  void update(std::vector<Matcher::p_match> p_matched, Matrix Tr,
              int32_t point_type = 1, int32_t min_track_length = 2,
              double max_dist = 30, double min_angle = 2);
 ...
 private:
  ...
  result updatePoint(const track &t, point3d &p, const FLOAT &step_size,
                     const FLOAT &eps);
  void computeObservations(const std::vector<point2d> &p);
  bool computePredictionsAndJacobian(
      const std::vector<Matrix>::iterator &P_begin,
      const std::vector<Matrix>::iterator &P_end, point3d &p);
  void testJacobian();
