...
Reconstruction::Reconstruction() { K = Matrix::eye(3); }
...
void Reconstruction::update(vector<Matcher::p_match> p_matched, Matrix Tr,
                            int32_t point_type, int32_t min_track_length,
                            double max_dist, double min_angle) {
  ...
  for (vector<Matcher::p_match>::iterator m = p_matched.begin();
       m != p_matched.end(); m++)
    if (m->i1p > track_idx_max) track_idx_max = m->i1p;
  for (vector<track>::iterator t = tracks.begin(); t != tracks.end(); t++)
    if (t->last_idx > track_idx_max) track_idx_max = t->last_idx;
  int32_t *track_idx = new int32_t[track_idx_max + 1];
  for (int32_t i = 0; i <= track_idx_max; i++) track_idx[i] = -1;
  for (int32_t i = 0; i < tracks.size(); i++) track_idx[tracks[i].last_idx] = i;
  ...
  for (vector<track>::iterator t = tracks_copy.begin(); t != tracks_copy.end();
       t++) {
      ...
      if (t->pixels.size() >= min_track_length) {
        ...
        if (initPoint(*t, p)) {
          if (pointType(*t, p) >= point_type)
            if (refinePoint(*t, p))
              if (pointDistance(*t, p) < max_dist &&
                  rayAngle(*t, p) > min_angle)
                points.push_back(p);
        }
      }
    }
...
double Reconstruction::rayAngle(const track &t, point3d &p) {
  ...
  FLOAT n1 = v1.l2norm();
  FLOAT n2 = v2.l2norm();
  if (n1 < 1e-10 || n2 < 1e-10) return 1000;
  v1 = v1 / n1;
  v2 = v2 / n2;
...
int32_t Reconstruction::pointType(const track &t, point3d &p) {
  ...
  // point not visible
  if (x1c.val[2][0] <= 1 || x2c.val[2][0] <= 1) return -1;

  // below road
  if (x2r.val[1][0] > 0.5) return 0;

  // road
  if (x2r.val[1][0] > -1) return 1;

  // obstacle
  return 2;
}

Reconstruction::result Reconstruction::updatePoint(const track &t, point3d &p,
  ...
  if (!computePredictionsAndJacobian(P_begin, P_end, p)) return FAILED;
  ...
  if (B.solve(A)) {
    ...
    if (fabs(B.val[0][0]) < eps && fabs(B.val[1][0]) < eps &&
        fabs(B.val[2][0]) < eps)
      return CONVERGED;
    else
      return UPDATED;
...
bool Reconstruction::computePredictionsAndJacobian(
    const vector<Matrix>::iterator &P_begin,
    const vector<Matrix>::iterator &P_end, point3d &p) {
  // for all frames do
  int32_t k = 0;
  for (vector<Matrix>::iterator P = P_begin; P <= P_end; P++) {
    // precompute coefficients
    FLOAT a = P->val[0][0] * p.x + P->val[0][1] * p.y + P->val[0][2] * p.z +
              P->val[0][3];
    FLOAT b = P->val[1][0] * p.x + P->val[1][1] * p.y + P->val[1][2] * p.z +
              P->val[1][3];
    FLOAT c = P->val[2][0] * p.x + P->val[2][1] * p.y + P->val[2][2] * p.z +
              P->val[2][3];
    FLOAT cc = c * c;

    // check singularities
    if (cc < 1e-10) return false;
...
void Reconstruction::testJacobian() {
  ...
  A.setMat(K, 0, 0);
  P.push_back(A);
  A.setMat(Matrix::rotMatX(0.1) * Matrix::rotMatY(0.1) * Matrix::rotMatZ(0.1),
           0, 0);
  A.val[1][3] = 1;
  A.val[1][3] = 0.1;
  A.val[1][3] = -1.5;
