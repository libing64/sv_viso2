...
vector<int32_t> VisualOdometryStereo::getInlier(
    vector<Matcher::p_match> &p_matched, vector<double> &tr) {
...
void VisualOdometryStereo::computeResidualsAndJacobian(
  ...
  for (int32_t i = 0; i < (int32_t)active.size(); i++) {
    ...
    p_predict[4 * i + 0] =
        param.calib.f * X1c / Z1c + param.calib.cu;  // left u
    p_predict[4 * i + 1] =
        param.calib.f * Y1c / Z1c + param.calib.cv;  // left v
    p_predict[4 * i + 2] =
