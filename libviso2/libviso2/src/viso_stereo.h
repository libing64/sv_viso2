...
class VisualOdometryStereo : public VisualOdometry {
 public:
  ...
  struct parameters : public VisualOdometry::parameters {
    ...
    bool reweighting;  // lower border weights (more robust to calibration
                       // errors)
    parameters() {
      base = 1.0;
      ransac_iters = 200;
  ...
  //         replace ... replace current images with I1 and I2, without copying
  //         last current
  //                     images to previous images internally. this option can
  //                     be used
  //                     when small/no motions are observed to obtain Tr_delta
  //                     wrt
  //                     an older coordinate system / time step than the
  //                     previous one.
 ...
 private:
  ...
  result updateParameters(std::vector<Matcher::p_match> &p_matched,
                          std::vector<int32_t> &active, std::vector<double> &tr,
                          double step_size, double eps);
  void computeObservations(std::vector<Matcher::p_match> &p_matched,
                           std::vector<int32_t> &active);
  void computeResidualsAndJacobian(std::vector<double> &tr,
                                   std::vector<int32_t> &active);
  std::vector<int32_t> getInlier(std::vector<Matcher::p_match> &p_matched,
