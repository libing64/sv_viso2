...
VisualOdometryMono::VisualOdometryMono(parameters param)
    : param(param), VisualOdometry((VisualOdometry::parameters)param) {}
...
vector<double> VisualOdometryMono::estimateMotion(
  ...
  if (N < 10) return vector<double>();

  // create calibration matrix
  double K_data[9] = {param.calib.f,  0, param.calib.cu, 0, param.calib.f,
                      param.calib.cv, 0, 0,              1};
  Matrix K(3, 3, K_data);
  ...
  for (int32_t k = 0; k < param.ransac_iters; k++) {
    ...
    if (inliers_curr.size() > inliers.size()) inliers = inliers_curr;
  }

  // are there enough inliers?
  if (inliers.size() < 10) return vector<double>();
  ...
  if (median > param.motion_threshold) return vector<double>();
...
Matrix VisualOdometryMono::smallerThanMedian(Matrix &X, double &median) {
  ...
  for (int32_t j = 0; j <= num_elem_half; j++)
    for (int32_t i = 0; i < 4; i++) X_small.val[i][j] = X.val[i][idx[j]];
  return X_small;
}

bool VisualOdometryMono::normalizeFeaturePoints(
    vector<Matcher::p_match> &p_matched, Matrix &Tp, Matrix &Tc) {
  ...
  if (fabs(sp) < 1e-10 || fabs(sc) < 1e-10) return false;
...
void VisualOdometryMono::fundamentalMatrix(
    const vector<Matcher::p_match> &p_matched, const vector<int32_t> &active,
    Matrix &F) {
...
vector<int32_t> VisualOdometryMono::getInlier(
    vector<Matcher::p_match> &p_matched, Matrix &F) {
  ...
  for (int32_t i = 0; i < (int32_t)p_matched.size(); i++) {
    ...
    // check threshold
    if (fabs(d) < param.inlier_threshold) inliers.push_back(i);
  }
...
void VisualOdometryMono::EtoRt(Matrix &E, Matrix &K,
                               vector<Matcher::p_match> &p_matched, Matrix &X,
                               Matrix &R, Matrix &t) {
  // hartley matrices
  ...
  for (int32_t i = 0; i < 4; i++) {
    int32_t num_inliers =
        triangulateChieral(p_matched, K, R_vec[i], t_vec[i], X_curr);
...
int32_t VisualOdometryMono::triangulateChieral(
    vector<Matcher::p_match> &p_matched, Matrix &K, Matrix &R, Matrix &t,
    Matrix &X) {
