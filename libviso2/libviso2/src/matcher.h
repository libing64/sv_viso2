...
class Matcher {
 public:
  ...
  struct parameters {
    ...
    int32_t
        multi_stage;  // 0=disabled,1=multistage matching (denser and faster)
    int32_t half_resolution;  // 0=disabled,1=match at half resolution, refine
                              // at full resolution
  ...
  // input: I1,I2 .......... pointers to left and right image (row-aligned),
  // range [0..255]
  //        dims[0,1] ...... image width and height (both images must be
  //        rectified and of same size)
  //        dims[2] ........ bytes per line (often equals width)
  //        replace ........ if this flag is set, the current image is
  //        overwritten with
  //                         the input images, otherwise the current image is
  //                         first copied
  //                         to the previous image (ring buffer functionality,
  //                         descriptors need
