...
class VisualOdometryMono : public VisualOdometry {
 public:
  ...
  //         replace ... replace current image with I, without copying last
  //         current
  //                     image to previous image internally. this option can be
  //                     used
 ...
 private:
  ...
  void fundamentalMatrix(const std::vector<Matcher::p_match> &p_matched,
                         const std::vector<int32_t> &active, Matrix &F);
  void EtoRt(Matrix &E, Matrix &K, std::vector<Matcher::p_match> &p_matched,
             Matrix &X, Matrix &R, Matrix &t);
  int32_t triangulateChieral(std::vector<Matcher::p_match> &p_matched,
                             Matrix &K, Matrix &R, Matrix &t, Matrix &X);
