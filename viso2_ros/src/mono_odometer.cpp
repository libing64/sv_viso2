...
class MonoOdometer : public OdometerBase {
 ...
 public:
  MonoOdometer(const std::string& transport) : OdometerBase(), replace_(false) {
    ...
    camera_sub_ = it.subscribeCamera("image", 1, &MonoOdometer::imageCallback,
                                     this, transport);
 ...
 protected:
  void imageCallback(const sensor_msgs::ImageConstPtr& image_msg,
    ...
    if (!visual_odometer_) {
      ...
      ROS_INFO_STREAM(
          "Initialized libviso2 mono odometry "
    ...
    if (first_run) {
      ...
      if (success) {
        ...
        ROS_DEBUG(
            "Call to VisualOdometryMono::process() failed. Assuming motion too "
            "small.");
        replace_ = true;
        tf::Transform delta_transform;
        delta_transform.setIdentity();
